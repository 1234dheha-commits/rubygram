...
//            }))
    ...
    // MARK: Rubygram — replaced Swiftgram Pro (paywalled) with always-visible Developer pane
    items[.swiftgram]!.append(PeerInfoScreenDisclosureItem(id: 0, label: swiftgramProLabel, text: "Developer", icon: PresentationResourcesSettings.swiftgramPro, action: {
        interaction.openSettings(.swiftgramPro)
    }))
    items[.swiftgram]!.append(PeerInfoScreenDisclosureItem(id: 1, label: swiftgramLabel, text: "Rubygram", icon: PresentationResourcesSettings.swiftgram, action: {
